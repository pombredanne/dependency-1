 ...
 * apidoc:0.9.48 http://www.apidoc.me/flow/common/0.0.2-dev/play_2_x_json
 */
...
package io.flow.common.v0.models {
  ...
  package object json {
    ...
    implicit def jsonReadsCommonOrganizationSummary: play.api.libs.json.Reads[OrganizationSummary] = {
      (__ \ "key").read[String].map { x => new OrganizationSummary(key = x) }
    }

    implicit def jsonWritesCommonOrganizationSummary: play.api.libs.json.Writes[OrganizationSummary] = new play.api.libs.json.Writes[OrganizationSummary] {
      def writes(x: OrganizationSummary) = play.api.libs.json.Json.obj(
    ...
    implicit def jsonReadsCommonSummary: play.api.libs.json.Reads[Summary] = {
      (
    ...
    implicit def jsonWritesCommonSummary: play.api.libs.json.Writes[Summary] = {
      (
        (__ \ "guid").write[_root_.java.util.UUID] and
        (__ \ "name").write[String]
      )(unlift(Summary.unapply _))
    }
  }
}
